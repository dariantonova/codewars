...
are positive or null integer (represented as strings) with one or two digits. Substrings in the
...
function msToHMS(ms) {
    ...
    const strArr = [h, m, s].map(n => n.toString().padStart(2, '0'));
    return strArr.join('|');
}

function stat(strg) {
    if (strg === '') return '';
    const results = strg.split(', ');
    const resultsMs = results.map(result => {
        const [h, m, s] = result.split('|');
        const ms = h * 60*60*1000 + m * 60*1000 + s * 1000;
        return ms;
    });

    const rangeMs = Math.max(...resultsMs) - Math.min(...resultsMs);
    const avgMs = resultsMs.reduce((sum, n) => sum + n) / resultsMs.length;

    const sortedResults = [...resultsMs].sort((a, b) => a - b);
    const middle = (results.length - 1) / 2;
    const medianMs = (sortedResults[Math.floor(middle)] + sortedResults[Math.ceil(middle)]) / 2;

    return `Range: ${msToHMS(rangeMs)} Average: ${msToHMS(avgMs)} Median: ${msToHMS(medianMs)}`;
}

const res = stat("1|25|11, 02|17|20, 12|17|20, 2|47|16");
console.log(res);