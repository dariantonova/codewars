...
Given a lottery ticket (ticket), represented by an array of 2-value arrays, you must find out if you've won
the jackpot.
...
To do this, you must first count the 'mini-wins' on your ticket. Each subarray has both a string and a number
...
function bingo(ticket, win){
    ...
    for (const subarr of ticket) {
        const [str, code] = subarr;
        const miniWin = str.split('').some(char => char.charCodeAt(0) === code);
        if (miniWin) miniWins++;
    }
