...
If the sum of a chunk's digits is divisible by 2, reverse that chunk; otherwise rotate it to
...
function revrot(str, sz) {
    ...
    const reg = new RegExp(`\\d{1,${sz}}`, 'g');
    return str.replace(reg, m => {
        if (m.length !== sz) return '';
        const sumOfDigits = m.split('').reduce((sum, n) => +n + sum, 0);
        if (sumOfDigits % 2 === 0) {
            return m.split('').reverse().join('');
        }
        else {
            return m.slice(1) + m.at(0);
        }
    });
}