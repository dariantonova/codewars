...
// ✅ SOLUTION
function getTownValues(town, strng) {
    const townStr = strng.split('\n').find((item) => item.startsWith(town + ':'));
    if (!townStr) return null;

    return townStr.split(':')[1].split(',').map(monthData => +monthData.split(' ')[1]);
}
...
function _variance(values) {
    ...
    return values.reduce((acc, n) => acc + (n - mean) ** 2, 0) / values.length;
}

function mean(town, strng) {
    const values = getTownValues(town, strng);
    if (!values) return -1;
...
const meanRes = mean(town, data);
const varianceRes = variance(town, data);

console.log('mean: ' + meanRes);
console.log('variance: ' + varianceRes);