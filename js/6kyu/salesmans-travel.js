...
Examples
r = "123 Main Street St. Louisville OH 43071,432 Main Long Road St. Louisville OH 43071,
786 High Street Pollocksville NY 56432"
...
function travel(r, zipcode) {
    const addresses = r.split(',').filter(address => address.endsWith(' ' + zipcode));
    const addressObjs = addresses.map(address => {
        const withoutZip = address.replace(zipcode, '');

        const [houseNumber, ...streetAndTownParts] = withoutZip.split(' ');
        const streetAndTown = streetAndTownParts.slice(0, -1).join(' ');
        return { houseNumber, streetAndTown };
    });

    const streetsAndTowns = addressObjs.map(a => a.streetAndTown).join(',');
    const houseNumbers = addressObjs.map(a => a.houseNumber).join(',');

    return `${zipcode}:${streetsAndTowns}/${houseNumbers}`;
}
